...
struct Motion {
    int32_t phase = 0; // world distance in hundredths, wraps at a material period
    int32_t position = 0; // hundredths along the closed track
    int32_t speed = 0; // world units/second, intentionally not calibrated km/h
    void tick(bool accelerate, bool brake) {
        speed += brake ? -4 : accelerate ? 2 : 0;
        if (speed < 0) speed = 0;
        if (speed > 300) speed = 300;
        position = (position + speed) % (TrackLength * 100);
        phase = (phase + speed) % (Period * 100); // one centisecond
    }
};
struct Stream {
    uint8_t data[4096];
...
struct Road {
    ...
    void project(int32_t position, int32_t phase) {
        ...
        for(int y=RoadTop;y<=Bottom+1;++y) {
            ...
            centers[y]=160L*256+lateral*(y-Horizon)/CameraHeight;
