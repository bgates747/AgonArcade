"""Capture a prepared RALLY-18 stock profile headlessly; never use for timing."""
...
REPO=TASK.parents[2]
SOURCE=TASK/'visual_capture_macos.c'
...
def build_library(output):
    library=output/'visual-capture.dylib'
    home=Path.home()
    command=['clang','-std=c11','-Wall','-Wextra','-Werror','-dynamiclib',
             f'-I{home}/.local/include',f'-L{home}/.local/lib','-lSDL3',str(SOURCE),'-o',str(library)]
...
def parse_actions(args):
    ...
    for item in filter(None,args.keys.split(',')):
        ...
        if name not in ('left','right','up','down','space','escape') or not 0<start<end:
...
def capture(args):
    ...
    if not profile.is_relative_to(TASK/'.emulator'):
        raise RuntimeError('The input profile must be isolated under RALLY-18/.emulator')
    output=args.output.resolve()
    if not output.is_relative_to(TASK):
        raise RuntimeError('Keep visual output within the RALLY-18 task bucket')
    ...
    # macOS strips DYLD_* when launching /bin/bash. The canonical wrapper uses
    # bash, so set this variable in its task-local BASH_ENV startup file.
    startup=output/'capture-env.sh'
    startup.write_text('export DYLD_INSERT_LIBRARIES="$RALLY_VISUAL_LIBRARY"\n')
    env=os.environ.copy()
    for key in list(env):
