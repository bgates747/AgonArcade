"""Interleave wide/word conversions across repeats and nested user calls."""
...
def main():
    ...
    source=(GOLEM/'examples/native_quantization/shared_scalar.golem').read_text()
    outputs=[]
    for variant,directive in [('isolated',''),('word','SharedWordScratch();\n'),('scalar','SharedScalarScratch();\n')]:
        path=TASK/'.work/shared-scalar-source'/a.name/variant;path.mkdir(parents=True,exist_ok=False)
        kernel=path/'kernel.golem';kernel.write_text(source.replace('SharedScalarScratch();\n',directive,1))
        root,raw=execute('golem-shared-scalar',a.name+'-'+variant,kernel,data,timeout=240)
