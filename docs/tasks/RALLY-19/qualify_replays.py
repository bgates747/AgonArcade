...
def main():
    ...
    for track,name in [('oval',a.oval),('fuji',a.fuji)]:
        ...
        for key,value in manifest['runtime_before'].items():assert manifest['runtime_after'][key]==value,key
        additions=set(manifest['runtime_after'])-set(manifest['runtime_before'])
        assert {Path(path).name for path in additions}=={'replay-summary.csv','memory.phase','replay-exit.snk','done.snk','start.snk','stop.snk','replay-readback.dat','go.snk'}
