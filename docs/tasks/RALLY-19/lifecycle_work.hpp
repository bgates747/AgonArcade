...
int r19Lifecycle(){
    ...
    uint8_t initial[12]{};if(!r19read::admission(initial))return 122;
    for(auto value:initial)if(value)return 122;
    ...
    for(unsigned cycle=0;cycle<3;++cycle){
        ...
        for(unsigned i=0;i<idCount;++i)if(!r19tag::collect(life,ids[i]))return 138;
