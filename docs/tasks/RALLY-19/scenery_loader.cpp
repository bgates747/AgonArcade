// Complete scene visual probe. Reference retains accepted viewport calls unchanged.
...
#endif
namespace {
#ifndef SCENERY_FRAMES_PER_POSE
#define SCENERY_FRAMES_PER_POSE 2
#endif
constexpr unsigned FramesPerPose=SCENERY_FRAMES_PER_POSE;
...
#ifdef VEHICLE_ORACLE
void drawScenery(volatile const rally::r18::SceneState &scene) {
...
int visual(){
    ...
    vdp_set_pixel_coordinates();vdp_reset_sprites();vdp_cursor_enable(false);artwork();loadScenery();
#ifdef VEHICLE_ORACLE
    ...
    for(unsigned i=0;i<count;++i){
...
#endif
        for(unsigned frame=0;frame<FramesPerPose;++frame){
#ifdef VEHICLE_ORACLE
...
#ifdef VEHICLE_ORACLE
        fprintf(out,"%d,%d,%d,%d,%d,%d,%d,%d,%u\n",int(scene.skyOffset),int(scene.skyDelta),int(scene.skyLeft),int(scene.skyRight),int(scene.skyRepaint),scene.playerBitmap,int(scene.playerMirror),scene.playerX,scene.cars);
        ...
        uint8_t sky[58];for(unsigned j=0;j<58;++j)if(!query(1740,j,sky[j]))return 25;
        auto skyWord=[&](unsigned offset){return rally::r19::word(sky+offset);};
        fprintf(out,"%u,%d,%u,%u,%u,%u,%u,%d,%u\n",unsigned(skyWord(38)),int(rally::r19::signed16(skyWord(24))),unsigned(skyWord(32)),unsigned(skyWord(34)),unsigned(skyWord(36)),unsigned(rally::r19::word(player)),unsigned(rally::r19::word(player+2)),int(rally::r19::signed16(rally::r19::word(player+4))),unsigned(rally::r19::word(flags+6)));
        ...
        if(!query(1002,4,lo)||!query(1002,5,hi)||lo+256u*hi!=FramesPerPose*(i+1))return 17;
