"""Reapply the frozen R19-10 budgets to the qualified stack-fix frontend.
...
from frontend_bridge import inline_assets
...
def main():
    ...
    root=TASK/'evidence/golem-performance'/a.name;root.mkdir(exist_ok=False)
    ...
    def includes(build):
        ...
        values={};inline_assets(work)
        for path in (work/'include').glob('*.hpp'):
            if path.name=='scene_assets.hpp':
                values[path.name]={'sha256':sha(path),'authoritative':'Recomputed by qualified inline asset bridge'}
                continue
    ...
    front=read(TASK/'evidence/golem-frontend'/(a.name+'-qualification.json'));assert front['pass'] and front['native_runs']==12
    assert front['audit_sha256']==sha(TASK/'qualify_inline_frontend.py')
    inline=read(TASK/'evidence/hardware-production/inline-candidate/frozen-qualification.json')
    assert inline['pass'] and inline['frozen_cases']==66
    inline_assets(Path(front['build']['work']))
    resources={}
    for track in ['oval','fuji']:
        path=Path(front['build']['work'])/(track+'.vdp.map')
        # Independent visual builds emitted the same map and bootstrap.
        assert path.read_bytes()==record(TASK/'evidence/golem-scenery-visual'/('hardware-inline-'+track)/'program.map').read_bytes()
        ledger={r.split()[0]:int(r.split()[1]) for r in record(path).read_text().splitlines() if r.split()[0] in RESOURCE_KEYS}
        ...
        assert ledger['bootstrap_bytes']==(Path(front['build']['work'])/(track+'.vdp')).stat().st_size
        resources[track]=ledger
    ...
    for phase in ['bridge','abba']:
        directory=TASK/'evidence/golem-render-work'/(a.name+'-'+phase)
        ...
        for track in ['oval','fuji']:
            assert sha(Path(saved['build']['work'])/(track+'.vdp'))==inline['candidate_bootstraps'][track]
    ...
    calibration_root=TASK/'evidence/golem-cpu-calibration'/a.name
    calibration=read(calibration_root/'results.json')
    ...
    cal_rows=integers(calibration_root/'cpu-calibration.csv')
    cal_log=record(calibration_root/'emulator.log').read_text()
    ...
    fragments=read(calibration_root/'identical-wrappers.json')
    for variant in ['measured','calibration']:
        listing=record(calibration_root/(variant+'-object.txt')).read_text()
    ...
    for track in ['oval','fuji']:
        ...
        for index,variant in enumerate(['oracle','golem','golem','oracle']):
            path=TASK/'evidence/golem-cpu-work'/f'{a.name}-abba-{track}-{index}-{variant}'
    ...
    report={'milestone':'R19-11 final-asset requalification of frozen R19-10 budgets','scope':__doc__,'pass':all(checks.values()),'checks':checks,'tracks':track_reports,
            'oracle_bridge':bridge_report,'runs':run_details,'resources':resources,'overhead':{'sink_empty_cycles':empty,'sink_call_upper_cycles':call,'observer':observer['summary']},
