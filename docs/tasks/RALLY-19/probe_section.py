...
def main():
    ...
    source=build(a.track,TASK/'.work/section-source');cases=queries(a.track);wanted=reference(a.track,cases)
    ...
    report={'pass':all(r['pass'] for r in results),'track':a.track,'scope':__doc__,'reference_sha256':sha(TASK/'section_reference.cpp'),'cases':len(results),'results':results}
