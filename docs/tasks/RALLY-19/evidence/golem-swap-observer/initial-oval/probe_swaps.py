...
def main():
    ...
    env.update(SDL_VIDEODRIVER='dummy',SDL_AUDIODRIVER='dummy',LD_PRELOAD=str(library),
               R19_VDP_MODULE=str(profile/'firmware/vdp_platform.so'),R19_SWAP_COUNT='67',
    ...
    assert len(rows)==134 and all(len(r)==67 for r in variants.values()),{k:len(v) for k,v in variants.items()}
    for kind,records in variants.items():
        assert [r['ordinal'] for r in records]==list(range(67))
    ...
    report={'scope':__doc__,'pass':True,'swaps':67,'startup_swap':1,'warmup_swaps':2,'workload_swaps':64,
            'events':134,'strict_nesting_and_sequence':True,'measurement':data,
