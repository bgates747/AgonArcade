...
int gameMain(int argc, char **argv) {
    ...
    if(!oracleRenderer && (perspective || computeOnly || displayTiming || snapshot ||
                           profiling || workload || fenced || noSky || noTraffic || mirrorPlayer || lightTiming)) {
    ...
    if(!measure||oracleRenderer||replayFrames<1||replayFrames>60000)return 115;
