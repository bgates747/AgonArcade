...
bool memoryProbe=false;
void memoryPhase(unsigned number) {
    if(!memoryProbe)return;
    FILE *f=fopen("memory.phase","w");if(f){fprintf(f,"%u\n",number);fclose(f);}
    // Give the independent SDL-thread sampler an idle, quiescent observation.
    uint32_t start=ticks();while(ticks()-start<30){}
}
int run() {
    if (vdp_mode(8) < 0)
        return 1;
    FILE *memory=fopen("memory.test","rb");memoryProbe=memory!=nullptr;if(memory)fclose(memory);
    ...
    for (unsigned cycle = 0; cycle < 3; ++cycle) {
        for (unsigned round = 0; round < 3; ++round) {
            ...
            if (!collect(all, 4100, 256))
                return 10;
            const unsigned offsets[]={0,255,256,32767,32768,65534};
            for(unsigned offset:offsets){uint8_t value;if(!query(4200,offset,value)||fwrite(&value,1,1,all)!=1)return 19;}
            memoryPhase(100+cycle*10+round);
            if(memoryProbe){
                for(unsigned i=0;i<4096;++i)call(2000);
                if(!poll(0xa5))return 17;
            }
            memoryPhase(200+cycle*10+round);
        }
    ...
    if(memoryProbe){
        memoryPhase(999);
        uint32_t start=ticks();
        while(!(vdp_getKeyMap(14)&1))if(ticks()-start>240)return 18;
    }
    vdp_adv_clear_buffer(Query);
    vdp_adv_clear_buffer(Foreign);
    if (!poll(0xa5))
