"""Audit preserved frontend source, native held controls and normal MOS exit.

R19-09 evidence only. This does not qualify timing, long stability or fault cleanup.
"""
...
def main():
    destination=TASK/'evidence/golem-frontend/qualification.json'
    ...
    for track in ['oval','fuji']:
        for case in ['demo','right','left','takeover','autosteer','oracle']:
            root=TASK/'evidence/golem-frontend'/f'{case}-{track}-fixed-default'
            inputs=read(root/'inputs.json');saved=read(root/'results.json')
            assert inputs['case']==case and inputs['track']==track
            build=inputs['build']
            if build_identity is None:build_identity=build
    ...
    report={'scope':__doc__,'pass':True,'native_runs':12,'host_state_records':638,
            ...
            'build':build_identity,'runs':runs,'evidence':evidence,
            'failures_retained':['demo-oval','startup-diagnostic'],
            'remaining':'R19-10 performance, R19-11 long stability/malformed/reset/lifecycle, R19-12 delivery.'}
