...
def capture(args):
    ...
    if not profile.is_relative_to(REPO/'.emulator'):
        raise RuntimeError('The input profile must be isolated under project .emulator')
    output=args.output.resolve()
    if not output.is_relative_to(REPO/'.research-cache/rally22'):
        raise RuntimeError('Keep visual output within the ignored RALLY-22 capture bucket')
